...
class StainPresetNet(nn.Module):
    def __init__(self, backbone='resnet18', resample_size=128, channels=8, layers=2):
        ...
        if backbone == 'resnet18':
            self.backbone_a = res.resnet18(num_classes=class_num)
            self.backbone_b = res.resnet18(num_classes=class_num)
        elif backbone == 'resnet34':
            self.backbone_a = res.resnet34(num_classes=class_num)
            self.backbone_b = res.resnet34(num_classes=class_num)
        elif backbone == 'resnet50':
            self.backbone_a = res.resnet50(num_classes=class_num)
            self.backbone_b = res.resnet50(num_classes=class_num)
        elif backbone == 'resnet101':
            self.backbone_a = res.resnet101(num_classes=class_num)
            self.backbone_b = res.resnet101(num_classes=class_num)
        else:
        ...
        self.style_fusion = torch.nn.Sequential(
            torch.nn.Linear(class_num * 2, class_num * 2),
            torch.nn.LeakyReLU(True),
            torch.nn.Linear(class_num * 2, class_num)
        )
        self.ref = None
    ...
    def set_ref(self, ref):
        assert ref.size(0) == 1, 'must single image'
        ref_resample = F.interpolate(ref, size=self.resample_size, mode="bilinear", align_corners=True)
        style_ref = self.backbone_b(ref_resample)
        self.ref = style_ref
        print('set reference image finish!')

    def style(self, x):
        x_resample = F.interpolate(x, size=self.resample_size, mode="bilinear", align_corners=True)
        style_x = self.backbone_a(x_resample)
        style_ref = self.ref.expand_as(style_x)
        out_weight = self.style_fusion(torch.cat([style_x, style_ref], dim=1))
        # out_weight = style_ref
